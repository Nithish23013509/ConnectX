...
@RestController
@RequestMapping("/api/apps/drive")
@RequiredArgsConstructor
public class GoogleDriveSignupController {
    ...
    @PostMapping("/connect")
    public ResponseEntity<?> connectDrive(
        ...
        ConnectedApp connectedApp = connectedAppRepository
                .findByUserAndProvider(user, "GOOGLE_DRIVE")
                .stream()
                .findFirst()
                .orElse(new ConnectedApp());

        connectedApp.setUser(user);
        connectedApp.setProvider("GOOGLE_DRIVE");
