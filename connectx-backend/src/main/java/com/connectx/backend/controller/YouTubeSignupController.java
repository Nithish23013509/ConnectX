...
@RestController
@RequestMapping("/api/apps/youtube")
@RequiredArgsConstructor
public class YouTubeSignupController {
    ...
    public ResponseEntity<?> connectYouTube(
        ...
        ConnectedApp connectedApp = connectedAppRepository
                .findByUserAndProvider(user, "YOUTUBE")
                .stream()
                .findFirst()
                .orElse(new ConnectedApp());

        connectedApp.setUser(user);
        connectedApp.setProvider("YOUTUBE");
        ...
        return ResponseEntity.ok("YouTube connected successfully");
    }
}
